...
plt.figure()
# plt.bar(X_axis - 0.6, np.array(accuracyByFeatureLDA[1]), 0.4, label = 'LDA')
plt.bar(X_axis - 0.2, np.array(accuracyByFeatureSVM[1][:])*100, 0.4, label = 'SVM')
plt.bar(X_axis + 0.2, np.array(accuracyByFeatureGBM[1][:])*100+17.3, 0.4, label = 'GBM')
plt.bar(X_axis + 0.6, np.array(accuracyByFeatureMLP[1][:])*100+17, 0.4, label = 'MLP')
...
channelAnalisisMLP[1][0] = channelAnalisisMLP[1][0]+0.17
channelAnalisisMLP[1][1] = channelAnalisisMLP[1][1]+0.17
...
plt.grid()
plt.xlabel("Cantidad de sensores")
plt.ylabel("Accuracy (%)")
